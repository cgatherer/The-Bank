...
// load Case Studies Post Types file
    require 'includes/post-types/register-casestudies.php';

// load Testimonials Post Types file
    require 'includes/post-types/register-testimonials.php';

// load Locations Post Types file
    require 'includes/post-types/register-locations.php';

// load Bios Post Types file
    require 'includes/post-types/register-bios.php';

// load Board Members Post Types file
    require 'includes/post-types/register-board.php';
...
function bootstrapwp_widgets_init() {
    ...
    register_sidebar(
        array(
            ...
            'before_widget' => '<div class="col-sm-2 col-xs-6">',
            'after_widget'  => '</div>',
            'before_title'  => '<h5>',
            'after_title'   => '</h5>'
        )
    ...
    register_sidebar(
        array(
            'name'          => __('Footer Content 2', 'bootstrapwp'),
            'id'            => 'footer-content-2',
            'description'   => __('Footer text or acknowledgements', 'bootstrapwp'),
            'before_widget' => '<div class="col-lg-12 col-md-12 col-sm-12">',
            'after_widget'  => '</div>',
            'before_title'  => '<h4>',
            'after_title'   => '</h4>'
        )
    ...
    function bootstrapwp_content_nav($nav_id) {
        ...
        <nav id="<?php echo $nav_id; ?>" class="navigation" role="navigation">
            ...
            <div class="nav-previous alignleft"><?php next_posts_link(
                __('<span class="meta-nav">←</span> Older posts', 'bootstrapwp')
            ); ?></div>
            <div class="nav-next alignright"><?php previous_posts_link(
                __('Newer posts <span class="meta-nav">→</span>', 'bootstrapwp')
            ); ?></div>
                        ...
                        <div class="media-body">
                            ...
                            <p class="reply">
                                <?php comment_reply_link( array_merge($args, array(
                                            'reply_text' => __('Reply <span>↓</span>', 'bootstrapwp'),
                                            'depth'      => $depth,
...
function getPartnerLinks(){
  $partner_links = pods( 'partner_links' );
  $partner_links->find();
  $url           = array();
  
  //Store all partner links in $url
  while( $partner_links->fetch() ):
    array_push( $url, str_replace('www.', '', parse_url( $partner_links->display( 'website' ), PHP_URL_HOST ) ) ); 
  endwhile;
  
  ?>
  	<script type="text/javascript">
    	var partnerLinks = [<?php echo '"'. implode( '","', $url ) . '"'; ?>];
    	...
    	jQuery('a').on('click', function(e){
      		...
      		else if( jQuery.inArray( domain, partnerLinks ) > -1 )
      		...
      		else if( domain != 'pgbank.com' && domain != '' )
